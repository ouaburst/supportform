...
if(isset($_POST['updateCategory'])){
	...
	if(!$DBhandler->updateCategory($categoryID,$categoryName));
...
<!-- 	<body> -->
	...
	<body onload="javascript:ListStatus()">
...
-->
		    ...
		    <div class="col-sm-4">
				...
				<a data-toggle='modal' href='#createCategoryModal' class='btn btn-primary' onclick="createCategoryModal()">Skapa en ny kategori</a>		
...
<!-- 	<form class="form-horizontal" method="post" action="index.php" id="createUserForm">         		 -->
	...
	<form class="form-horizontal" method="post" action="category.php" id="block-validate">         				
		    ...
		    <div class="modal-dialog">
		      <div class="modal-content">
		        <div class="modal-header">
		          ...
		          <h4 class="modal-title">Skapa en ny kategori</h4>
		        </div>
			        ...
			        <div class="modal-footer">
			          ...
			          <button class="btn btn-primary" type="submit" name="saveCategory">Spara</button>
			        </div>
...
<!--  ---------------------------- Update user modal --------------------------------------- -->
	...
	<form class="form-horizontal" method="post" action="category.php" id="block-validate02">         				
	  ...
	  <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	    <div class="modal-dialog">
	      <div class="modal-content">
	        <div class="modal-header">
	          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
	          <h4 class="modal-title">Uppdatera kategori</h4>
	        </div>
	        <div class="modal-body">
	        ...
	        <div class="modal-footer">
	          <button type="button" class="btn btn-default" data-dismiss="modal">St&auml;ng</button>
			  <button class="btn btn-primary" type="submit" name="updateCategory">Uppdatera</button>		          
	        </div>
	      </div><!-- /.modal-content -->
	    </div><!-- /.modal-dialog -->
	  </div><!-- /.modal -->
...
<!--  ---------------------------- /Email setup  modal --------------------------------------- -->
		...
		function createCategoryModal(){
			...
			$.ajax({                              
		      url: 'ajax.php', 
			  type: "POST",			      
		      data: ({createCategoryModal: createCategoryModal}),             			                            
		...
		function editCategoryModal(categoryID){
			...
			$.ajax({                              
		      url: 'ajax.php', 
			  type: "POST",			      
		      data: ({editCategoryModal: editCategoryModal, categoryID: categoryID}),             			                            
		      success: function(data) 
		      {
		...
		function deleteCategory(categoryID) { 
		    ...
		    bootbox.confirm("Vill du verkligen radera kategorin?", $.proxy(function(result) {
		        if (result) {
				...
				$.ajax({                                      
				  ...
				  type: "POST",		
				  async: "false",	      
			      data: ({deleteCategory: deleteCategory, categoryID: categoryID}),             			                         
			      success: function(data) 
			      {
					  ListStatus();	
			      } 
		...
		function ListStatus() { 		
			...
			$('#usersTable').dataTable( {
				...
				"fnServerParams": function ( aoData ) {
				      aoData.push( { "name": "getCategory", "value": "getCategory" } );
				},    
				...
				"aoColumns": [ {"bVisible": false}, null, 
				  ...
				  { 
				    ...
				    "fnRender": function (oObj) {
				    	var operString = "";
				    	operString += " <a role='button' class='btn btn-danger btn-sm deleteCategory' href='#' onclick=\"deleteCategory('"+oObj.aData[0]+"')\"><i class='glyphicon glyphicon-remove'></i></a>";		       
				    	operString += " <a data-toggle='modal' href='#editModal' class='btn btn-primary btn-sm' onclick=\"editCategoryModal('"+oObj.aData[0]+"')\"><i class='glyphicon glyphicon-pencil'></i></a>";
				    	return operString;
				     }
				  }
