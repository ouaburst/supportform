...
if(isset($_POST['savePrio'])){

	$prioName = $_POST['prioname']; 
	
	$DBhandler->addPrio($prioName);			
...
if(isset($_POST['updatePrio'])){
	...
	if(!$DBhandler->updatePrio($prioID,$prioName));
...
<!-- 	<body> -->
	...
	<body onload="javascript:listPrio()">
...
-->
		    ...
		    <div class="col-sm-4">
		    	...
		    	<h4>Hantera prioritet</h4>
				<a data-toggle='modal' href='#createPrioModal' class='btn btn-primary' onclick="createPrioModal()">Skapa en ny prioritet</a>		
...
<!-- 	<form class="form-horizontal" method="post" action="index.php" id="createUserForm">         		 -->
	...
	<form class="form-horizontal" method="post" action="priority.php" id="block-validate">         				
	
		  <div class="modal fade" id="createPrioModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		    <div class="modal-dialog">
		      <div class="modal-content">
		        <div class="modal-header">
		          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
		          <h4 class="modal-title">Skapa en ny prioritet</h4>
		        </div>
			        ...
			        <div class="modal-footer">
			          ...
			          <button class="btn btn-primary" type="submit" name="savePrio">Spara</button>
			        </div>
...
<!--  ---------------------------- Update prio modal --------------------------------------- -->
	...
	<form class="form-horizontal" method="post" action="priority.php" id="block-validate02">         				
	  ...
	  <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	    <div class="modal-dialog">
	      <div class="modal-content">
	        <div class="modal-header">
	          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
	          <h4 class="modal-title">Uppdatera Prioriteten</h4>
	        </div>
	        <div class="modal-body">
	        ...
	        <div class="modal-footer">
	          <button type="button" class="btn btn-default" data-dismiss="modal">St&auml;ng</button>
			  <button class="btn btn-primary" type="submit" name="updatePrio">Uppdatera</button>		          
	        </div>
	      </div><!-- /.modal-content -->
	    </div><!-- /.modal-dialog -->
	  </div><!-- /.modal -->
...
<!--  ---------------------------- /Email setup  modal --------------------------------------- -->
		...
		function createPrioModal(){
			...
			var createPrioModal = "createPrioModal";
				
			$.ajax({                              
		      url: 'ajax.php', 
			  type: "POST",			      
		      data: ({createPrioModal: createPrioModal}),             			                            
		...
		function editPrioModal(prioID){
			...
			console.log("prioID: "+prioID);
		
			var editPrioModal = "editPrioModal";
				
			$.ajax({                              
		      url: 'ajax.php', 
			  type: "POST",			      
		      data: ({editPrioModal: editPrioModal, prioID: prioID}),             			                            
		      success: function(data) 
		      {
		...
		function deletePrio(prioID) { 
		
		    bootbox.confirm("Vill du verkligen radera prioriteten?", $.proxy(function(result) {
		        if (result) {

				console.log("prioID: "+prioID);				            							

				var deletePrio = "deletePrio";
			
				$.ajax({                                      
			      url: 'ajax.php', 
				  type: "POST",		
				  async: "false",	      
			      data: ({deletePrio: deletePrio, prioID: prioID}),             			                         
			      success: function(data) 
			      {
					  listPrio();	
			      } 
		...
		function listPrio() { 		
			...
			$('#usersTable').dataTable( {
				...
				"fnServerParams": function ( aoData ) {
				      aoData.push( { "name": "getPrio", "value": "getPrio" } );
				},    
				...
				"aoColumns": [ {"bVisible": false}, null, 
				  ...
				  { 
				    ...
				    "fnRender": function (oObj) {
				    	var operString = "";
				    	operString += " <a role='button' class='btn btn-danger btn-sm deletePrio' href='#' onclick=\"deletePrio('"+oObj.aData[0]+"')\"><i class='glyphicon glyphicon-remove'></i></a>";		       
				    	operString += " <a data-toggle='modal' href='#editModal' class='btn btn-primary btn-sm' onclick=\"editPrioModal('"+oObj.aData[0]+"')\"><i class='glyphicon glyphicon-pencil'></i></a>";
				    	return operString;
				     }
				  }
